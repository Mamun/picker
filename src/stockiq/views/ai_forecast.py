...
def render_ai_forecast(gaps_df: pd.DataFrame, show_share_btn: bool = True) -> None:
    """Render the AI 10-day SPY forecast table. Pass the computed gaps DataFrame."""
    ...
    app_key   = _get_secret(env_var)

    # ── Personal key input (always shown so users can bypass quota limits) ────
    user_key = _personal_key_input(provider)

    # Effective key: personal key wins over app key
    active_key = user_key or app_key

    if not active_key:
        st.warning(
            f"No API key found for **{prov_info['label']}**. "
            f"Set `{env_var}` or paste your own key above.",
            icon="🔑",
        )
        st.caption(
    ...
    context_json = build_forecast_context(gaps_df, quote, daily_df=daily_df, vix_df=vix_df, pc_data=pc_data)

    with st.spinner(f"Generating AI forecast via {prov_info['label']}…"):
        try:
            predictions = fetch_ai_prediction(cache_key, context_json, provider=provider, _user_key=user_key)
